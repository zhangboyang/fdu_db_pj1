...
function query_statistics()
{
    ...
    request_data({
        action: "getadminorderstatistics",
        statstart: qstart,
        statend: qend,
    }).then( function (data) {
        if (data.result != "ok") {
            show_error("getadminorderstatistics error: " + data.reason);
            return;
        }
        ...
        show_error("can't admin order statistics: " + reason);
    });
}
...
function show_order_detail(oitem)
{
    ...
    $("#od_restaddr").text(oitem.orestaddr);
    $("#od_resttel").text(oitem.oresttel);
